...
class TestMultisig(TrezorTest):
    ...
    #     with pytest.raises(CallException):
    #         self.client.sign_tx('Bitcoin', [inp1], [out1, out2_ok])
    ...
    def test_change(self):
        ...
        with self.client:
            self.client.set_expected_responses([
                ...
                # not a change - needs to be confirmed
            ...
            # Now we have first signature
            (signatures1, _) = self.client.sign_tx('Bitcoin', [inp1], [out1, out2])
        ...
        #         proto.HDNodePathType(node=node, address_n=[1]),
        #         proto.HDNodePathType(node=node, address_n=[2]),
        #         proto.HDNodePathType(node=node, address_n=[10])  # added malicious pubkey
        #     ],
        #     signatures=[b'', b'', b''],
        #     m=2,
        # )
